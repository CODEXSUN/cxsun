...
## What We Done
- Added tenant-aware sequential Contact code previews and save-time fallback generation through `GET /api/v1/contacts/next-code`.
- Backed automatic Contact codes with a tenant-local sequence and transaction lock so concurrent creates receive distinct values.
- Added MariaDB mutation coverage for the full location chain, independent Pincode persistence, Contact reload, and concurrent code allocation.
