...
export class TallyService {
  ...
  private async salesSyncList(
    ...
    const entries = await listSalesEntries(context)
  ...
  private async purchaseSyncList(
    ...
    const entries = await listPurchaseEntries(context)
  ...
  private async syncSales(context: TenantRuntimeContext, ids: string[]) {
    const settings = await this.requireValidatedSettings(context)
    const links = await this.linkMap(context, ['contacts', 'products', 'sales'])
    const entries = (await listSalesEntries(context)).filter((entry) => ids.includes(String(entry.uuid)))
    const summary = { failed: 0, synced: 0 }
  ...
  private async queuePurchase(context: TenantRuntimeContext, ids: string[]) {
    ...
    const links = await this.linkMap(context, ['contacts', 'products', 'purchase'])
    const entries = (await listPurchaseEntries(context)).filter((entry) => ids.includes(String(entry.uuid)))
...
async function listSalesEntries(context: TenantRuntimeContext): Promise<AnyRow[]> {
  ...
  return Promise.all(rows.map(async (row: AnyRow) => ({
    ...row,
    items: await database
      .selectFrom('sales_entry_items')
      .selectAll()
      .where('sales_entry_id', '=', Number(row.id))
      .orderBy('sort_order', 'asc')
      .execute(),
  })))
}

async function listPurchaseEntries(context: TenantRuntimeContext): Promise<AnyRow[]> {
  const database = context.database as any
  const rows = await database
  ...
  return Promise.all(rows.map(async (row: AnyRow) => ({
    ...
    items: await database
      ...
      .selectAll()
      .where('purchase_entry_id', '=', Number(row.id))
      .orderBy('sort_order', 'asc')
      .execute(),
  })))
}
