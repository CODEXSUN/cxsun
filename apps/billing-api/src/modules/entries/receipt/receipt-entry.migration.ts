...
import type { TenantDatabaseSchema } from '@cxsun/platform/infrastructure/tenant-database/tenant-database.schema.js'

type TenantDatabase = Kysely<TenantDatabaseSchema>

export async function migrateReceiptEntryTables(database: TenantDatabase) {
  await sql`
    CREATE TABLE IF NOT EXISTS receipt_entries (
      ...
      receipt_mode VARCHAR(80) NOT NULL,
      ...
      UNIQUE KEY uq_receipt_entries_context_no (tenant_id, company_id, accounting_year_id, receipt_no),
      INDEX idx_receipt_entries_tenant_date (tenant_id, receipt_date, id),
      INDEX idx_receipt_entries_company_year (company_id, accounting_year_id)
    )
  `.execute(database)

  await sql`
    CREATE TABLE IF NOT EXISTS receipt_entry_allocations (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      INDEX idx_receipt_allocations_entry (receipt_entry_id, sort_order)
    )
  `.execute(database)

  await sql`
    CREATE TABLE IF NOT EXISTS receipt_entry_comments (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
      receipt_entry_id BIGINT UNSIGNED NOT NULL,
      uuid VARCHAR(80) NOT NULL,
      ...
      body TEXT NOT NULL,
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_receipt_comments_entry (receipt_entry_id, id)
    )
  `.execute(database)

  await sql`
    CREATE TABLE IF NOT EXISTS receipt_entry_activities (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
      receipt_entry_id BIGINT UNSIGNED NOT NULL,
      uuid VARCHAR(80) NOT NULL,
      ...
      payload JSON NULL,
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_receipt_activities_entry (receipt_entry_id, id)
    )
  `.execute(database)
}
