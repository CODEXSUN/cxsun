...
import type { TenantDomain, TenantDomainUpsertInput } from './tenant-domain.types.js'
...
export class TenantDomainRepository {
  ...
  async resolve(hostOrDomain: string) {
    ...
    return getDatabase()
      ...
      .select([
        ...
        'tenants.db_name',
        'tenants.db_user',
        'tenants.db_secret_ref',
        'tenants.company_count',
        'tenants.active_company_count',
        'tenants.company_concept_count',
  ...
  async upsert(input: RequiredTenantDomainInput): Promise<TenantDomain> {
    ...
    if (input.id) {
      await getDatabase().updateTable('tenant_domains').set(row).where('id', '=', input.id).execute()
      const updated = await this.findById(input.id)
      if (!updated) throw new Error('Tenant domain update did not return a persisted domain.')
      return updated
    }

    await getDatabase().insertInto('tenant_domains').values(row).execute()
    const created = await this.findByDomain(input.domain)
    if (!created) throw new Error('Tenant domain insert did not return a persisted domain.')
    return created
  }
  ...
  async deleteById(id: number): Promise<boolean> {
    const result = await getDatabase().deleteFrom('tenant_domains').where('id', '=', id).executeTakeFirst()
...
export type RequiredTenantDomainInput = Required<Pick<TenantDomainUpsertInput, 'domain' | 'label' | 'status'>> & {
  id?: number
  tenant_id: number
  is_primary: number
  settings: string
