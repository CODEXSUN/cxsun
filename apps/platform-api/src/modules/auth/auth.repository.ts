...
import type { AdminUserRecord, PlatformUserStatus, PlatformUserUpsertInput, TenantUserRecord, TenantUserSummary } from './auth.types.js'

@Injectable()
export class AuthRepository {
  findAdminUserByEmail(email: string) {
    return getDatabase().selectFrom('admin_users').select(['id', 'name', 'email', 'password_hash', 'role', 'status']).where('email', '=', email).executeTakeFirst()
  }
  ...
  findAdminUserById(id: number) {
    return getDatabase().selectFrom('admin_users').select(['id', 'name', 'email', 'role', 'status']).where('id', '=', id).executeTakeFirst()
  }
  ...
  async upsertAdminUser(input: RequiredAdminUserInput): Promise<AdminUserRecord | undefined> {
    ...
    if (input.id) {
      ...
      await getDatabase().updateTable('admin_users').set(update).where('id', '=', input.id).execute()
      return (await this.listAdminUsers()).find((user) => user.id === input.id)
    }

    if (!input.password) return undefined
    await getDatabase().insertInto('admin_users').values({ ...row, password_hash: hashPassword(input.password) }).execute()
    const created = await getDatabase().selectFrom('admin_users').select('id').where('email', '=', input.email).executeTakeFirstOrThrow()
    return (await this.listAdminUsers()).find((user) => user.id === created.id)
  }
  ...
  findTenantByLoginIdentifier(identifier: string): Promise<Tenant | undefined> {
    ...
    let query = getDatabase().selectFrom('tenants').selectAll().where('tenants.deleted_at', 'is', null)
    query = normalizedMobile
      ? query.where((eb) => eb.or([eb('tenants.corporate_id', '=', normalizedCorporateId), eb('tenants.mobile', '=', normalizedMobile)]))
  ...
  findTenantBySlug(slug: string): Promise<Tenant | undefined> {
    return getDatabase().selectFrom('tenants').selectAll().where('slug', '=', slug).where('deleted_at', 'is', null).executeTakeFirst() as Promise<Tenant | undefined>
  }

  async listUserTenantSummaries(): Promise<TenantUserSummary[]> {
    const tenants = await getDatabase().selectFrom('tenants').selectAll().orderBy('tenants.code', 'asc').execute() as Tenant[]
    const summaries: TenantUserSummary[] = []
    for (const tenant of tenants) {
      let userCount = 0
      try {
        const row = await getTenantDatabase(tenant).selectFrom('users').select(({ fn }) => fn.count<number>('id').as('user_count')).executeTakeFirst()
        userCount = Number(row?.user_count ?? 0)
      } catch {
        userCount = 0
      }
      summaries.push({ tenant_id: tenant.id, tenant_code: tenant.code, tenant_slug: tenant.slug, tenant_name: tenant.name, tenant_status: tenant.status, user_count: userCount })
    }
    return summaries
  }
  ...
  async listUsersByTenant(tenantId: number): Promise<TenantUserRecord[]> {
    ...
    const rows = await getTenantDatabase(tenant)
      ...
      .select(['user_tenants.id as access_id', 'users.id as user_id', 'users.name', 'users.email', 'user_tenants.role', 'users.status', 'users.created_at', 'users.updated_at', 'user_tenants.created_at as access_created_at'])
  ...
  async findUserById(tenantId: number, userId: number) {
    ...
    return getTenantDatabase(tenant).selectFrom('users').select(['id', 'name', 'email', 'status']).where('id', '=', userId).executeTakeFirst()
  }
  ...
  async upsertTenantUser(input: RequiredPlatformUserInput) {
    ...
    if (userId) {
      const update: { name: string; email: string; status: PlatformUserStatus; updated_at: Date; password_hash?: string } = {
        name: input.name,
        email: input.email,
        status: input.status,
        updated_at: new Date(),
      ...
      if (!input.password) return undefined
      await database.insertInto('users').values({ uuid: nextLocalUuid(), name: input.name, email: input.email, password_hash: hashPassword(input.password), status: input.status, updated_at: new Date() }).execute()
    ...
    const existingAccess = input.access_id
      ...
      : await database.selectFrom('user_tenants').select('id').where('user_id', '=', userId).executeTakeFirst()
    if (existingAccess) {
    ...
    return (await this.listUsersByTenant(input.tenant_id)).find((row) => row.user_id === userId)
  }

  private findTenantById(tenantId: number): Promise<Tenant | undefined> {
    return getDatabase().selectFrom('tenants').selectAll().where('id', '=', tenantId).executeTakeFirst() as Promise<Tenant | undefined>
  }
}
