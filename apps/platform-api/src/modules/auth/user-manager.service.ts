...
import { AuthRepository, type RequiredAdminUserInput, type RequiredPlatformUserInput } from './auth.repository.js'
import type { AdminUserUpsertInput, PlatformUserStatus, PlatformUserUpsertInput } from './auth.types.js'
import { TenantContextService, type TenantRequestHeaders } from './tenant-context.service.js'
...
export class UserManagerService {
  ...
  async upsertAdminUser(headers: TenantRequestHeaders, input: AdminUserUpsertInput) {
    ...
    if (duplicate) return { ok: false, error: 'Email is already used by another admin user.' }

    const existingUser = normalized.id ? await this.users.findAdminUserById(normalized.id) : undefined
    if (normalized.id && !existingUser) return { ok: false, error: 'Admin user was not found.' }
    ...
    if (existingUser?.role === 'super-admin' && (normalized.role !== 'super-admin' || normalized.status !== 'active')) {
      ...
      if (activeSuperAdminCount <= 1) return { ok: false, error: 'At least one active super admin is required.' }
    }

    if (normalized.id === actor.sub && normalized.status !== 'active') return { ok: false, error: 'You cannot suspend your own admin account.' }
    const user = await this.users.upsertAdminUser(normalized)
    return user ? { ok: true, user } : { ok: false, error: 'Admin user save failed.' }
  }
  ...
  async listTenantUsers(headers: TenantRequestHeaders, tenantId: number) {
    ...
    if (!platformAccess) {
      ...
      if (context.tenant.id !== tenantId) throw new ForbiddenException('User list is limited to the authenticated tenant.')
    }
  ...
  async upsert(headers: TenantRequestHeaders, input: PlatformUserUpsertInput) {
    ...
    if (normalized.user_id && !existingUser) return { ok: false, error: 'User was not found.' }

    const user = await this.users.upsertTenantUser(normalized)
    return user ? { ok: true, user } : { ok: false, error: 'User save failed.' }
  }
  ...
  private async normalize(input: PlatformUserUpsertInput): Promise<RequiredPlatformUserInput | { ok: false; error: string }> {
    ...
    return { access_id: accessId, user_id: userId, tenant_id: tenantId, name, email, password, role, status }
  }
  ...
  private requireSuperAdminAccess(headers: TenantRequestHeaders) {
    ...
    if (!auth || auth.role !== 'super-admin') throw new ForbiddenException('Super-admin access is required.')
    return auth
  }

  private requirePlatformUserManagerAccess(headers: TenantRequestHeaders) {
    if (!this.platformAccess(headers)) throw new ForbiddenException('Platform user-manager access is required.')
  }

  private async requireTenantUserManagerAccess(headers: TenantRequestHeaders, tenantId: number) {
    if (this.platformAccess(headers)) return
    const context = await this.tenantContext.resolve(headers)
    if (context.tenant.id !== tenantId) throw new ForbiddenException('User management is limited to the authenticated tenant.')
    if (!['admin', 'manager', 'software-admin'].includes(context.user.role)) throw new ForbiddenException('Tenant admin or manager access is required.')
  }
  ...
  private platformAccess(headers: TenantRequestHeaders): AuthTokenPayload | undefined {
    ...
    if (auth.identitySource !== 'platform') return undefined
