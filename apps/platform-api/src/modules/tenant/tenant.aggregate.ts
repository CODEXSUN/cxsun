...
export class TenantAggregate {
  static normalize(input: TenantUpsertInput, nextCode: number): TenantUpsertData {
    ...
    if (!Number.isInteger(code) || code < 100) throw new TenantValidationError('Tenant code must be an integer starting from 100.')
    if (!name) throw new TenantValidationError('Tenant name is required.')
    if (!isTenantStatus(status)) throw new TenantValidationError('Tenant status is invalid.')
    if (!slug) throw new TenantValidationError('Tenant slug is required.')
    if (!corporateId) throw new TenantValidationError('Corporate ID is required.')
    if (!isJsonObject(payloadSettings)) throw new TenantValidationError('Tenant payload settings must be a JSON object.')
    if (!dbName) throw new TenantValidationError('Tenant database name is required.')
    if (!dbHost) throw new TenantValidationError('Tenant database host is required.')
    if (!Number.isInteger(dbPort) || dbPort <= 0) throw new TenantValidationError('Tenant database port is invalid.')
    if (!dbUser) throw new TenantValidationError('Tenant database user is required.')
    if (!dbSecretRef) throw new TenantValidationError('Tenant database secret reference is required.')
