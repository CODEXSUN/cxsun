...
import type { TallySettings, TallySettingsInput, TallySyncItem, TallySyncJob, TallySyncJobInput, TallyWorkspace } from './tally.types.js'
...
export class TallyRepository {
  ...
  async saveSettings(context: TenantRuntimeContext, input: TallySettingsInput): Promise<TallySettings> {
    ...
    const patch = {
      ...
      tally_host: input.tally_host?.trim() || 'localhost',
      tally_port: numberValue(input.tally_port) || 9000,
      company_name: emptyAsNull(input.company_name),
      sync_sales: input.sync_sales ?? true,
      sync_purchase: input.sync_purchase ?? true,
      sync_receipt: input.sync_receipt ?? true,
      sync_payment: input.sync_payment ?? true,
      sync_inventory: input.sync_inventory ?? false,
      sync_contacts: input.sync_contacts ?? true,
      sync_direction: input.sync_direction?.trim() || 'export',
      settings: jsonOrNull(input.settings) ?? current.settings,
      updated_by: context.user.email,
      updated_at: new Date(),
    }
  ...
  async createJob(context: TenantRuntimeContext, input: TallySyncJobInput): Promise<TallyWorkspace> {
    ...
    const jobType = input.job_type?.trim() || 'manual-sync'
