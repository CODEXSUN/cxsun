...
type DesignerPage = { sections: BroadcastSection[] }
type View = { mode: "list" } | { mode: "section" } | { mode: "show"; card: BroadcastCard } | { mode: "upsert"; card: BroadcastCard | null }
...
export function BroadcastSectionDesignerPage({ onNotice }: { onNotice(message: string): void }) {
  ...
  useEffect(() => {
    load().catch(() => onNotice("Unable to load the broadcast section."))
  }, [])

  async function saveSection(input: SectionForm) {
    if (!section?.uuid) return
    await adminApi<BroadcastSection>(`/frontend-designer/sections/${section.uuid}`, {
      method: "PUT",
      body: JSON.stringify({
        pageKey: "home",
        sectionKey: "broadcast-section",
        sectionType: "broadcast-flow",
    ...
    await load()
    onNotice("Broadcast section updated.")
    setView({ mode: "list" })
  }

  async function saveCard(input: CardForm, card: BroadcastCard | null) {
    if (!section?.uuid) return
    const saved = await adminApi<BroadcastCard>(
    ...
    await load()
    onNotice(card ? "Broadcast step updated." : "Broadcast step created.")
  ...
  return (
    <section className="slider-page rfq-section-page">
      <PageHeader
        ...
        description="Manage the public buyer requirement broadcast flow."
        actions={(
          <>
            ...
            <button className="admin-button primary" onClick={() => setView({ mode: "upsert", card: null })} type="button"><Plus size={16} />New step</button>
          </>
        )}
      />
      <BroadcastPreview section={section} cards={filtered} />
      <div className="slider-toolbar">
        <input onChange={(event) => setSearch(event.target.value)} placeholder="Search key, label, title, or body" value={search} />
      ...
      <div className="slider-table-wrap">
        <table className="slider-table">
          <thead><tr><th>#</th><th>Step</th><th>Label</th><th>Status</th><th>Order</th><th>Updated</th><th className="align-right">Action</th></tr></thead>
          <tbody>
              ...
              <tr key={card.uuid}>
                ...
                <td className="align-right">
                  <div className="slider-row-menu">
                    <button className="icon-action" onClick={() => setOpenAction((current) => current === card.uuid ? null : card.uuid)} title="Step actions" type="button"><MoreHorizontal size={16} /></button>
        ...
        {!filtered.length ? <div className="slider-empty">{loading ? "Loading broadcast steps." : "No broadcast steps found."}</div> : null}
      </div>
      <div className="slider-list-footer"><span>Total steps: <strong>{filtered.length}</strong></span><span>{cards.filter((item) => item.status === "active").length} active</span></div>
    </section>
  )
}

function BroadcastPreview({ cards, section }: { cards: BroadcastCard[]; section?: BroadcastSection }) {
...
function CardShow({ card, onBack, onEdit }: { card: BroadcastCard; onBack(): void; onEdit(): void }) {
  return (
    <section className="slider-page rfq-section-page">
      <PageHeader title={card.title || "Broadcast step"} description={`${card.item_key} - Order ${card.sort_order} - ${card.status}`} back={onBack} actions={<button className="admin-button primary" onClick={onEdit} type="button"><Pencil size={16} />Edit step</button>} />
      <BroadcastPreview cards={[card]} />
      <div className="slider-detail-grid">
        <Detail label="Label" value={card.eyebrow} />
...
function SectionUpsert({ onBack, onSubmit, section }: { onBack(): void; onSubmit(input: SectionForm): Promise<void>; section?: BroadcastSection }) {
  ...
  return (
    <section className="slider-page rfq-section-page">
      <PageHeader title="Edit broadcast section" description="Update public copy and tone for buyer requirement broadcast." back={onBack} />
...
function CardUpsert({ card, onBack, onSubmit }: { card: BroadcastCard | null; onBack(): void; onSubmit(input: CardForm): Promise<void> }) {
  ...
  return (
    <section className="slider-page rfq-section-page">
      <PageHeader title={card ? "Edit broadcast step" : "New broadcast step"} description="Manage one public broadcast flow step." back={onBack} />
      <form className="slider-upsert" onSubmit={submit}>
        <div className="slider-form-card">
          <h3 className="slider-form-heading">Step content</h3>
          <div className="slider-form-grid three">
            <InputField label="Step key" required value={form.itemKey} onChange={(itemKey) => setForm({ ...form, itemKey })} />
            <InputField label="Order" type="number" value={form.sortOrder} onChange={(sortOrder) => setForm({ ...form, sortOrder })} />
            <SelectField label="Status" value={form.status} options={["active", "draft", "archived"]} onChange={(status) => setForm({ ...form, status })} />
          </div>
          <InputField label="Label" required value={form.eyebrow} onChange={(eyebrow) => setForm({ ...form, eyebrow })} />
          <InputField label="Title" required value={form.title} onChange={(title) => setForm({ ...form, title })} />
          <div className="slider-form-grid two">
            <SelectField label="Icon" value={form.icon} options={["send", "scan-search", "bell-ring", "handshake"]} onChange={(icon) => setForm({ ...form, icon })} />
        ...
        <div className="slider-form-card">
          ...
          <BroadcastPreview cards={[formToCard(form, card)]} />
        </div>
        <div className="slider-form-actions">
          <button className="admin-button" onClick={onBack} type="button"><ArrowLeft size={16} />Cancel</button>
          <button className="admin-button primary" disabled={saving} type="submit"><Save className={saving ? "spin" : ""} size={16} />Save step</button>
        </div>
      </form>
    </section>
  )
...
function sectionToForm(section?: BroadcastSection): SectionForm {
  return {
...
function cardToForm(card: BroadcastCard | null): CardForm {
  return {
    itemKey: card?.item_key ?? `broadcast-step-${Date.now()}`,
...
function formToCard(form: CardForm, card: BroadcastCard | null): BroadcastCard {
  return {
    uuid: card?.uuid ?? "preview",
    item_key: form.itemKey,
    eyebrow: form.eyebrow,
    title: form.title || "Broadcast step title",
    summary: null,
    body: form.body,
    image_url: null,
