...
type DesignerPage = { sections: MarketplaceSection[] }
type View = { mode: "list" } | { mode: "section" } | { mode: "show"; card: MarketplaceCard } | { mode: "upsert"; card: MarketplaceCard | null }
...
export function MarketplaceSectionDesignerPage({ onNotice }: { onNotice(message: string): void }) {
  ...
  useEffect(() => {
    load().catch(() => onNotice("Unable to load the Marketplace section."))
  }, [])

  async function saveSection(input: SectionForm) {
    if (!section?.uuid) return
    await adminApi<MarketplaceSection>(`/frontend-designer/sections/${section.uuid}`, {
      method: "PUT",
      body: JSON.stringify({
        pageKey: "home",
        sectionKey: "marketplace-section",
    ...
    await load()
    onNotice("Marketplace section updated.")
    setView({ mode: "list" })
  }

  async function saveCard(input: CardForm, card: MarketplaceCard | null) {
    if (!section?.uuid) return
    const saved = await adminApi<MarketplaceCard>(
    ...
    await load()
    onNotice(card ? "marketplace card updated." : "marketplace card created.")
  ...
  return (
    <section className="slider-page marketplace-section-page">
      <PageHeader
        ...
        description="Manage the public Textile Marketplace section and its catalog cards."
        actions={(
          <>
      ...
      />
      <MarketplacePreview section={section} cards={filtered} />
      ...
      <div className="slider-table-wrap">
        ...
        {!filtered.length ? <div className="slider-empty">{loading ? "Loading marketplace cards." : "No marketplace cards found."}</div> : null}
      </div>
...
function MarketplacePreview({ cards, section }: { cards: MarketplaceCard[]; section?: MarketplaceSection }) {
  const form = sectionToForm(section)
  return (
    <div className={`marketplace-preview tone-${form.tone}`}>
      <div>
...
function CardShow({ card, onBack, onEdit }: { card: MarketplaceCard; onBack(): void; onEdit(): void }) {
  return (
    <section className="slider-page marketplace-section-page">
...
function SectionUpsert({ onBack, onSubmit, section }: { onBack(): void; onSubmit(input: SectionForm): Promise<void>; section?: MarketplaceSection }) {
  ...
  return (
    <section className="slider-page marketplace-section-page">
      <PageHeader title="Edit Marketplace section" description="Update public copy and tone for the catalog card area." back={onBack} />
...
function CardUpsert({ card, onBack, onSubmit }: { card: MarketplaceCard | null; onBack(): void; onSubmit(input: CardForm): Promise<void> }) {
  ...
  return (
    <section className="slider-page marketplace-section-page">
      <PageHeader title={card ? "Edit marketplace card" : "New marketplace card"} description="Manage one public marketplace catalog card." back={onBack} />
      <form className="slider-upsert" onSubmit={submit}>
        <div className="slider-form-card">
        ...
        <div className="slider-form-card">
          ...
          <MarketplacePreview cards={[formToCard(form, card)]} />
        </div>
        <div className="slider-form-actions">
          <button className="admin-button" onClick={onBack} type="button"><ArrowLeft size={16} />Cancel</button>
...
function sectionToForm(section?: MarketplaceSection): SectionForm {
  return {
...
function cardToForm(card: MarketplaceCard | null): CardForm {
  return {
    itemKey: card?.item_key ?? `marketplace-card-${Date.now()}`,
...
function formToCard(form: CardForm, card: MarketplaceCard | null): MarketplaceCard {
  return {
