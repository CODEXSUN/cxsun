...
## What We Done
- Contact addresses persist the exact selected Pincode independently from city and expose inline Pincode creation through the shared autocomplete.
- Address lookup creation follows the active Country → State → District → City chain, so newly created child records remain attached to the parent selected in the current Contact or address popup.
- Contact lists show Contact first and Code second, with only the contact name in the Contact cell. New-contact forms load the next tenant code (`C-0001`, `C-0002`, and so on), auto-capitalize Legal name from Name until the user overrides it, and keep the Active control compact on the right.
- Contact address rows use a clear two-column sequence: Address Type, full-width Address line 1, full-width Address line 2, Country/State, District/City, then Pincode/Default address.
