...
function EmailRow({ item, onChange, onRemove }: { item: ContactEmail; onChange(patch: Partial<ContactEmail>): void; onRemove(): void }) { return <Row onRemove={onRemove}><Field label="Email" value={item.email} onChange={(email) => onChange({ email })} /><EnumSelectField<ContactEmailType> label="Email type" options={contactEmailTypes} value={item.emailType} onChange={(emailType) => onChange({ emailType })} /><Primary checked={item.isPrimary} label="Primary email" onChange={(isPrimary) => onChange({ isPrimary })} /></Row> }
function PhoneRow({ item, onChange, onRemove }: { item: ContactPhone; onChange(patch: Partial<ContactPhone>): void; onRemove(): void }) { return <Row onRemove={onRemove}><Field label="Phone" value={item.phoneNumber} onChange={(phoneNumber) => onChange({ phoneNumber })} /><EnumSelectField<ContactPhoneType> label="Phone type" options={contactPhoneTypes} value={item.phoneType} onChange={(phoneType) => onChange({ phoneType })} /><Primary checked={item.isPrimary} label="Primary phone" onChange={(isPrimary) => onChange({ isPrimary })} /></Row> }
