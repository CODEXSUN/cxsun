...
function PaymentContactCreateDialog({ contacts, initialName, onClose, onCreated, session }: { contacts: PaymentLookupOption[]; initialName: string; onClose(): void; onCreated(contact: PaymentLookupOption): void; session: AuthSession }) {
  const autoSupplierCode = nextSupplierContactCode(contacts)
  const [draft, setDraft] = useState<ContactInput>(() => ({ ...emptyContact(), code: autoSupplierCode, contactTypeId: "contact-type:supplier", ledgerId: "ledger:sundry-creditors", ledgerName: "Supplier", legalName: titleCaseName(initialName), name: initialName }))
  const [error, setError] = useState<string | null>(null)
  const contactTypesQuery = useQuery(paymentQueries.contactTypesDialog(session))
  const nextCodeQuery = useQuery({ queryKey: ["payment-contact-next-code", session.selectedTenant.slug], queryFn: () => getNextContactCode(session) })
  const nextSupplierCode = nextSupplierContactCode(contacts, nextCodeQuery.data)
  useEffect(() => {
    setDraft((current) => {
      const currentCode = String(current.code ?? "").trim()
      if (currentCode && currentCode !== autoSupplierCode) return current
      return { ...current, code: nextSupplierCode }
    })
  }, [autoSupplierCode, nextSupplierCode])
  ...
  async function save() {
    ...
    setError(null)
    await createMutation.mutateAsync({ ...draft, code: String(draft.code ?? "").trim() || nextSupplierCode, contactTypeId: stockContactTypeId(contactTypes, "supplier"), gstin, ledgerId: draft.ledgerId ?? "ledger:sundry-creditors", ledgerName: draft.ledgerName ?? "Supplier", legalName: String(draft.legalName ?? "").trim() || titleCaseName(name), name })
  }
    ...
    <div className="fixed inset-0 z-[160] grid place-items-center bg-background/55 p-4 backdrop-blur-sm">
      <div className="w-[min(560px,calc(100vw-2rem))] overflow-hidden rounded-md border border-border/70 bg-card shadow-xl">
        ...
        <div className="grid gap-4 p-5 md:grid-cols-2">
          <Field label="Supplier name *" value={String(draft.name ?? "")} onChange={(name) => setDraft((current) => ({ ...current, name, legalName: shouldAutoFillPaymentLegalName(current) ? titleCaseName(name) : current.legalName }))} />
...
function nextSupplierContactCode(contacts: PaymentLookupOption[], contactNextCode?: string) {
  const numbers = contacts
    .map((contact) => prefixedCodeNumber(String(contact.code ?? contact.record.code ?? ""), "S"))
    .filter((value): value is number => value !== null)
  const contactSequenceNumber = prefixedCodeNumber(String(contactNextCode ?? "").replace(/^C-/i, "S-"), "S")
...
function shouldAutoFillPaymentLegalName(contact: ContactInput) {
