...
if [ ! -e "$APP_DIR/apps/frontend/public/storage" ]; then
  ln -s "$APP_DIR/storage/public" "$APP_DIR/apps/frontend/public/storage" 2>/dev/null || mkdir -p "$APP_DIR/apps/frontend/public/storage"
fi
...
set_env_optional "SUPER_ADMIN_NAME" "${SUPER_ADMIN_NAME:-}"
set_env_optional "SUPER_ADMIN_EMAIL" "${SUPER_ADMIN_EMAIL:-}"
set_env_optional "SUPER_ADMIN_PASSWORD" "${SUPER_ADMIN_PASSWORD:-}"
set_env_optional "SOFTWARE_ADMIN_NAME" "${SOFTWARE_ADMIN_NAME:-}"
set_env_optional "SOFTWARE_ADMIN_EMAIL" "${SOFTWARE_ADMIN_EMAIL:-}"
set_env_optional "SOFTWARE_ADMIN_PASSWORD" "${SOFTWARE_ADMIN_PASSWORD:-}"
set_env_optional "TENANT_ADMIN_NAME" "${TENANT_ADMIN_NAME:-}"
set_env_optional "TENANT_ADMIN_EMAIL" "${TENANT_ADMIN_EMAIL:-}"
set_env_optional "TENANT_ADMIN_PASSWORD" "${TENANT_ADMIN_PASSWORD:-}"
...
=======
echo "Configured ports: backend=$SERVER_PORT frontend=$FRONTEND_PORT"
echo "Configured URLs: api=$API_BASE_URL frontend=$FRONTEND_APP_URL storage=$STORAGE_BASE_URL media=$MEDIA_MANAGER_URL"
>>>>>>> Stashed changes
...
if [ "$SKIP_MARIADB_WAIT" = "true" ]; then
  ...
  for attempt in $(seq 1 60); do
    if mysqladmin ping --host="$DB_HOST" --port="$DB_PORT" --user="$DB_USER" --password="$DB_PASSWORD" --silent >/dev/null 2>&1; then
      echo "MariaDB is reachable"
      break
    fi
    if [ "$attempt" -eq 60 ]; then
      echo "MariaDB was not reachable after waiting. Database setup will perform the real connection check." >&2
      break
    fi
    sleep 2
  done
...
if [ "$QUEUE_RUNTIME_ENABLED" != "false" ]; then
  ...
  for attempt in $(seq 1 "$HEALTH_WAIT_SECONDS"); do
    ...
    if [ "$attempt" -eq "$HEALTH_WAIT_SECONDS" ]; then
      echo "Redis was not reachable after waiting. Queue workers will fall back when supported." >&2
      break
    fi
    sleep 1
  done
...
if [ "$INSTALL_RUN_TESTS" = "true" ]; then
  ...
  log_step "Skipping install-time tenant safety tests"
fi
...
if [ "$AUTO_SEED_TENANT_DOMAINS" = "true" ]; then
  log_step "Checking tenant resolver"
