...
For the cleaned billing workspace, keep backend routes on `6005` and point `/` at the main frontend port:

```bash
sudo codexsun tirupurconnect.com --ssl --www --frontend-port 6010 --force
sudo codexsun aaranassociates.com --ssl --www --frontend-port 6010 --force
```
