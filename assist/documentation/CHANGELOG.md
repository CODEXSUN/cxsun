...
- Removed stale root-level local/runtime folders, obsolete Docker/container scaffolding that referenced deleted split-service workspaces, desktop-only env keys, and inactive workspace-map entries so the repository root reflects the active server/frontend app.
...
- Split CXSync into two isolated modes: Maintenance Upgrade for manual full-database clone, candidate upgrade, verification, backup/upload, and cutover-later work; and Mirror for future scheduled online-to-offline incremental replication.
- Added dedicated Maintenance Upgrade and Mirror pages to the CXSync navigation, with Mirror exposing protected foundation status while keeping the incremental scheduler unarmed until cursor and audit rules are approved.
- Added separate Desktop and Cloud mirror operational tables for mirror jobs and cursors so future online-to-offline replication does not share Maintenance Upgrade batch, backup, or execution tables.
- Added the first manual CXSync Mirror Full Sync flow: CXSync Cloud creates a full tenant dump in isolated mirror storage, Desktop downloads it, restores it into a dedicated local mirror database such as `cxmirror_<tenantCode>`, verifies table and row counts, and records a full-dump cursor without enabling scheduled incremental replication.
- Hardened Mirror Full Sync by enforcing `cxmirror_` local targets, persisting Desktop phase/history/details, persisting Cloud dump evidence with expiry metadata, exposing recent full-sync history in the Mirror page, and verifying every table row count instead of only aggregate totals.
- Added an all-tenant Mirror full-sync queue for Desktop that runs saved tenant connections serially, reports completed/failed counts, and keeps each tenant on its own `cxmirror_*` target.
- Added a local daily Mirror schedule for the office server that runs the all-tenant full-sync queue only while CXSync Desktop is open and skips overlapping runs.
- Added manual CXSync Mirror incremental pull for one tenant after full bootstrap: Cloud returns eligible `updated_at` rows from single-primary-key tables, Desktop upserts them into the dedicated `cxmirror_*` database, and Desktop saves per-table `updated-at-json` cursors for the next pull. Deletes, schema migration, and daily incremental scheduling remain future hardening.
- Added all-tenant Mirror incremental queue and daily schedule mode selection, so the office server can run either full bootstrap or incremental cloud-to-local pulls serially for all saved tenants.
- Hardened Mirror incremental runs with a required verified full-dump bootstrap cursor, persisted incremental run history, non-null source database cursors, and summary evidence for source database, eligible tables, pulled tables, and pulled rows.
- Added a repeatable `npm -w apps/cxsync-cloud run test:mirror-e2e` smoke that creates a protected Cloud full dump, restores it into a disposable `cxmirror_e2e_*` database, verifies per-table row counts, runs a small incremental upsert check, and drops the disposable database by default.
- Completed the Mirror incremental flow with table coverage reporting, skipped-table reasons, paged catch-up until no more rows are available, a safety page cap, and Desktop audit JSON export support. Delete propagation remains intentionally gated on a real tombstone/outbox or reliable soft-delete source marker.
- Closed the current CXSync Mirror flow as complete for safe online-to-offline office replication: full bootstrap, all-tenant queue, scheduled incremental pull, coverage evidence, audit export support, and repeatable E2E verification are now recorded as the release baseline.
- Added safe delete propagation through an explicit tenant `cxsync_mirror_tombstones` outbox: Cloud returns tombstones and delete cursor status when the outbox exists, Desktop applies matching deletes to `cxmirror_*`, and Mirror reports `missing-tombstone-outbox` instead of guessing deletes when no outbox is present.
- Split the CXSync UI into two breadcrumb-switchable desks: **CXSync Sync** for maintenance/tenant/SQL dump/diagnostics/service-key work, and **CXSync Mirror** for mirror operations with its own side menu and setup links.
- Reworked the CXSync Mirror desk into a clean list/show flow: the list page shows tenant mirror cards, all-tenant queue controls, and schedule settings; each tenant show page owns its target database form, full/incremental actions, progress cards, and scoped history.
- Reworked the Mirror daily schedule into a clean upsert form with run time, mode, active/paused summary, and an enable/disable switch instead of separate enable/disable buttons.
- Reworked Mirror Sync with top-level Full Sync, Start, Pause, Stop, and Refresh controls for the all-tenant queue; queue cards now show aggregate progress without remaining/queued text.
- Moved Mirror schedule upsert into its own Mirror Settings page with the enable/disable switch, keeping live sync controls separate from scheduling configuration.
- Added per-tenant Mirror progress cards on the list page showing full/incremental status, local mirror target, status detail, and progress bar.
- Flattened Mirror Sync into a Super Admin-style tenant table with one tenant per row, moving controls into the top header row and removing dashboard count cards from the live sync page.
- Added compact colored Mirror state chips for current runtime status and saved schedule status, plus icon-led table status pills with shorter operator text.
## v-1.0.127
